# hcom Orchestration For `do-start`

Use this reference when `do-start` encounters concrete `Execution` metadata and must orchestrate headless workers from the current session.
