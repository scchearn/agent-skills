...
Plans generated by `do-plan` begin with YAML front matter and a mirrored `## Plan summary` section in the markdown body.

Tracked metadata:

- `title`
- `slug`
- `description` (70 tokens max)
- `status`
- `task_count`
- `created_at`
- `updated_at`
...
`do-start` and `do-amend` are expected to keep those values synchronized with the plan body and `plans/INDEX.md`.
